...
class TinyCStrParser(Parser):
    ...
    @_('REAL_CONST')
    def expr(self, p):
        return Const(p.REAL_CONST, DataType.DOUBLE)
    @_('CHAR_CONST')
    def expr(self, p):
        return Const(p.CHAR_CONST, DataType.CHAR)
    @_('STRING_CONST')
    def expr(self, p):
        return Const(p.STRING_CONST, DataType.STRING)
    @_('expr LT expr')
    def expr(self, p):
        return RelOp('<', p.expr0, p.expr1)
    ...
    def expr(self, p):
        return Cast(DataType.DOUBLE, p[3])

    @_('LPAREN INT RPAREN expr %prec CAST')
    def expr(self, p):
        return Cast(DataType.INT, p[3])
